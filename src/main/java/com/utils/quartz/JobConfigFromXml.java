	...
	public static List<JobConfig> loadConfig(String configFile) {
		...
		try {
			...
			for (Element jobElement : jobElements) {
				...
				if (Trigger.TRIGGER_TYPE_DAILY.equalsIgnoreCase(type)) {
					temp.setTriggerType(Trigger.TRIGGER_TYPE_DAILY);
					temp.setStartHour(Integer.valueOf(trigger
							.elementText(E_HOUR)));
					temp.setStartMinute(Integer.valueOf(trigger
							.elementText(E_MINUTE)));
				} else if (Trigger.TRIGGER_TYPE_HOURLY.equalsIgnoreCase(type)) {
					temp.setTriggerType(Trigger.TRIGGER_TYPE_HOURLY);
				...
				} else if (Trigger.TRIGGER_TYPE_MINUTELY.equalsIgnoreCase(type)) {
					temp.setTriggerType(Trigger.TRIGGER_TYPE_MINUTELY);
					if (StringUtils.isNotBlank(trigger
							.elementText(E_DELAY_MINUTE))) {
						temp.setDelayMinute(Integer.valueOf(trigger
								.elementText(E_DELAY_MINUTE)));
